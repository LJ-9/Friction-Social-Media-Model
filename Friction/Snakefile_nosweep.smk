""" How to run: change into dir: marketplace/example
    Run using `snakemake`. (specify filename when calling in terminal if not called Snakefile)
...
rule init_net:
    input:
        ...
        follower = os.path.join(DATA_DIR, 'Amended_BA_Friction_m=3_n=1000_coeff=.29.gml'),
        configfile = os.path.join(DATA_DIR, 'config.json')
